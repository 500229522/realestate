...
$agent = $conn->query("SELECT * FROM users u join agents a on u.id = a.user_id where u.id =$user_id ");

$arr = $agent->fetch_array();
...
<div class="card card-outline rounded-0 card-primary">
	<div class="card-body">
		<div class="container-fluid">
			...
			<form action="" id="agent-profile">	
                ...
                <div class="row">
                        <div class="col-md-6">
                            <label for="designation" class="control-label">Designation<sup></sup></label>
                            <input name="designation" id="designation" class="form-control form-control-sm rounded-0"  value="<?=isset($designation) ? $designation : "" ?>"></input>
                        </div>
                        <div class="col-md-6">
                            <label for="agency_name" class="control-label">Agency Name<sup></sup></label>
                            <input name="agency_name" id="agency_name" class="form-control form-control-sm rounded-0" value="<?=isset($agency_name) ? $agency_name : "" ?>"></input>
                        </div>
                    </div>
				<hr>
				<div class="row">
	...
	<div class="card-footer">
			<div class="col-md-12">
				<div class="row">
					<button class="btn btn-sm btn-primary" form="agent-profile">Update</button>
				</div>
			</div>
		</div>
</div>
...
<script>
	...
	$('#agent-profile').submit(function(e){
        e.preventDefault();
        var _this = $(this)
            $('.err-msg').remove();
        var el = $('<div>')
        ...
        start_loader();
        $.ajax({
            url:_base_url_+"classes/profile.php?f=update_agent",
